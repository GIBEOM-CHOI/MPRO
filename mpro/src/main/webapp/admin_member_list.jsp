...
<section id="reviewContainer">
		<table>
			<tr>
				<th colspan="7"><h1> 일반 회원정보 수정</h1></th>
			</tr>
			<tr>
				<th>아이디</th>
				<th>이름</th>
			...
			<c:forEach var="list" items="${ memberList }">
				<tr>
					...
					<td>${ list.address }</td>
					<td>
					<form action="/mpro/admin/edit_member.do" method="post">
					<input type="hidden" name="member_id" value="${ list.id }">
					<input type="hidden" name="member_password" value="${ list.password }">
					<input type="submit" value="수정하기">
					</form>
					</td>
					<td>
					<form action="/mpro/admin/delete_member.do" method="post">
		...
		<div style="text-align:center;">
			<c:if test="${ begin != 1 }"><a href="/mpro/admin/list_member.do?nowPage=${ begin - 1 }">[이전]</a></c:if>
		        <c:forEach begin="${ begin }" end="${ end }" var="page">
						<a href="/mpro/admin/list_member.do?nowPage=${ page }">[ ${ page } ]</a>
		        </c:forEach>
		    <c:if test="${ end != lastPage }"><a href="/mpro/admin/list_member.do?nowPage=${ end + 1 }">[다음]</a></c:if>
			</div>
	</section>
  ...
  <div class="footer">
    <div class="footer_sec">
      ...
      <div class="logo_and_mall">
        ...
        <!-- 입점신청 -->
        <div>
          <a href="#"><input type="submit" value="입점신청" class="mall_admin"></a></input>
        </div>
