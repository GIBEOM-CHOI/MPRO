		...
		try {
			if (action.equals("/join.do")) {
				...
				if(member.getId() != null) {
					if("30".equals(member.getAuthority_id()) || ("20".equals(member.getAuthority_id()) && "1".equals(member.getEntry())) || "10".equals(member.getAuthority_id())) {
...
//				out.close();
				...
				String id = request.getParameter("id");
				String password = request.getParameter("pw");
				String passwordC = request.getParameter("pwC");
				String name = request.getParameter("name");
				String tel = request.getParameter("tel");
				String address = request.getParameter("address");
				PrintWriter out = response.getWriter();
				if("".equals(name) || name == null ||
						...
						"".equals(tel) || tel == null ||
						"".equals(address) || address == null) {
					out.println("<script>alert('모든 내용을 입력해주세요.'); location.href='/mpro/member/member_mod.do';</script>");
					out.close();
				}else if(password.equals(passwordC)) {
						MemberDTO dto = new MemberDTO();
						dto.setId((String)session.getAttribute("id"));
						dto.setPassword((String)session.getAttribute("pw"));
						dto.setName(request.getParameter("name"));
						dto.setTel(request.getParameter("tel"));
						dto.setAddress(request.getParameter("addresss"));
