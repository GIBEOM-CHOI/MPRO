...
public class MylistDAO {
	...
	public MylistDTO selectMemberReviewC(String id, String code){ //리뷰 쓸 수 있는 회원인지 찾는 메소드
		MylistDTO dto = new MylistDTO();
		
		try {
			con = dataFactory.getConnection();
			
			String query = "select m.id, m.authority_id, l.mylist_id, l.order_date, l.code, l.sort"
					+ "    from tb_member m, tb_mylist l"
					+ "    where m.id = l.id"
					+ "    and l.code like '" + code + "'"
					+ "    and m.authority_id = 30"
					+ "    and m.id like '" + id + "'";
			pstmt = con.prepareStatement(query);
			ResultSet result = pstmt.executeQuery();
			while(result.next()) {
